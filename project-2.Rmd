...
model_comp <- cbind(cv_lr$CV_loss, cv_lda$CV_loss, cv_qda$CV_loss)
colnames(model_comp) <- c("LR", "LDA", "QDA")
model_comp %>%
  kable(digits = 3,
...
#  mutate(Label = tl) %>%
#  filter(Label != 0)
#blabel <- btrain %>% pull(Label)
#btrain = btrain %>% dplyr::select(!Label)

#btest = block_test %>%
#  mutate(Label = block_test_labels) %>%
#  filter(Label != 0)
#btest_label = btest %>% pull(Label)
#btest = btest %>% dplyr::select(!c(Label, block))

#CVmaster(btrain, blabel, "lda",
#         K = 8, split = "done", type = "class", formula = TRUE,
#         test_data = btest, test_labels = btest_label)
...
cv_qda_b <- CVmaster(X_cv[, -c(4, 6, 8, 9, 11, 12)], y_cv, "qda",
                   K = 8, split = "block", type = "class")
source("CVmaster.R")
cv_mlp_b <- CVmaster(X_cv[, -c(4, 6, 8, 9, 11, 12)], y_cv, "tree",
                   K = 8, split = "block", type = "vector")

model_comp_b <- cbind(cv_lr_b$CV_loss, cv_lda_b$CV_loss, 
                      cv_qda_b$CV_loss, cv_mlp_b$CV_loss)
colnames(model_comp_b) <- c("LR", "LDA", "QDA", "MLP")
model_comp_b %>%
  kable(digits = 3,
        caption = "CV Model Comparison") %>%
  kable_styling(latex_options = "HOLD_position", full_width = F)
...
ggplot() +
  ...
  geom_line(aes(x = 1 - cv_mlp_b$roc_obj$specificities,
                y = cv_mlp_b$roc_obj$sensitivities,
                color = paste0("QDA AUC = ", round(cv_mlp_b$roc_obj$auc, 3)))) +
  labs(title = "Test ROC Model Comparison",
       x = "1 - Specificity", y = "Sensitivity",
       color = "ROC curves")
```
