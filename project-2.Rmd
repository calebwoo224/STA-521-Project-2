...
#tl = c(block_train_labels, block_valid_labels)
#td = rbind(block_train, block_valid)

#CVmaster(td, tl, "qda", K = 8, split = "done", type = "class")
```
...
```{r cv-model-comparison, message=FALSE}
source("CVmaster.R")
...
colnames(model_comp) <- c("LR", "LDA", "QDA", "Tree", "10-NN", "RF")
model_comp %>%
  kable(digits = 3,
        caption = "CV Model Comparison") %>%
  kable_styling(latex_options = "HOLD_position", full_width = F)
```
...
ggplot() +
  ...
  labs(title = "Test ROC Model Comparison",
...
colnames(model_comp_b) <- c("LR", "LDA", "QDA", "Tree", "10-NN", "RF")
model_comp_b %>%
  kable(digits = 3,
        caption = "CV Model Comparison") %>%
  kable_styling(latex_options = "HOLD_position", full_width = F)
...
## Trying to get XGBoost working

```{r}
#dtrain <- xgb.DMatrix(data = as.matrix(X_cv[, -c(1:4, 6, 8, 9, 11, 12)]), label = X_cv$Label)
ex_boost <- xgboost(data = as.matrix(X_cv[, -c(1:4, 6, 8, 9, 11, 12)]), label = X_cv$Label,
                    nrounds = 2, objective = "binary:logistic", verbose = 0)
```

```{r}
predict(ex_boost, as.matrix(X_cv[, -c(1:4, 6, 8, 9, 11, 12)]), type="response")
```

Invalid input data when using CVmaster.R

```{r}
source("CVmaster.R")
CVmaster(X_cv[, -c(4, 6, 8, 9, 11, 12)], X_cv$Label, "xgboost", formula = F,
         K = 8, split = "image", type = "response",
         nrounds = 2, nthread = 2,
         objective = "binary:logistic", verbose = 0)
```

## Trying to get AdaBoost working

```{r}
ex_ada <- boosting(y_cv ~ NDAI + CORR + BF, data = cbind(X_cv, y_cv), boos = F, mfinal = 2,
                   control = rpart.control(maxdepth = 1))
```

```{r}
ada_preds <- predict(ex_ada, X_cv[, -c(1:4, 6, 8, 9, 11, 12)], type="prob")
ada_preds
```

Weight vector different variable length when using CVmaster.R

```{r}
source("CVmaster.R")
CVmaster(X_cv[, -c(4, 6, 8, 9, 11, 12)], y_cv, "boosting",
                   K = 8, split = "image", type = "prob",
                   boos = F, mfinal = 2,
         control = rpart.control(maxdepth = 1))
```
