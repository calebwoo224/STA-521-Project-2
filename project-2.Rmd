...
```{r val-test-maps}
split_ims <- image_split(data[,-4], data[,4], 5)
...
```{r lr-split-comparison, message=FALSE}
source("CVmaster.R")
X_cv <- data %>%
  filter(Label != 0) %>%
  mutate(Label = ifelse(
...
image_lr <- CVmaster(X_cv[, -c(4, 6, 8, 9, 11, 12)], y_cv, "glm",
                       K = 8, split = "image", type = "response",
                       thresh=0.5, family="binomial")
block_lr <- CVmaster(X_cv[, -c(4, 6, 8, 9, 11, 12)], y_cv, "glm",
                       K = 8, split = "block", type = "response",
                       thresh=0.5, family="binomial")

lr_comp <- cbind(image_lr$CV_loss, block_lr$CV_loss)
colnames(lr_comp) <- c("Image Split LR", "Block Split LR")
lr_comp %>%
  kable(digits = 3,
        caption = "Logistic Regression Split Comparison") %>%
  kable_styling(latex_options = "HOLD_position", full_width = F)
```
...
model_comp <- cbind(cv_lr$CV_loss, cv_lda$CV_loss, 
                    ...
                    cv_knn$CV_loss)
colnames(model_comp) <- c("LR", "LDA", "QDA", "Tree", "10-NN")
model_comp %>%
  kable(digits = 3,
...
model_comp_b <- cbind(cv_lr_b$CV_loss, cv_lda_b$CV_loss, 
                      ...
                      cv_knn_b$CV_loss)
colnames(model_comp_b) <- c("LR", "LDA", "QDA", "Tree", "10-NN")
model_comp_b %>%
  kable(digits = 3,
        caption = "CV Model Comparison") %>%
  kable_styling(latex_options = "HOLD_position", full_width = F)
