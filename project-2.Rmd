...
```{r block-split-baseline}
bsplit = block_split(X_cv, y_cv, K=8)
...
bbks = sample(bks)
btnum = floor(length(bks)/6) #Set ~1/6 for testing
...
for (f in folds) {
  ...
  freq <- mean(y != 0)
  image_split_baseline <- c(image_split_baseline, freq)
}
block_split_baseline <- c(block_split_baseline,
                          mean(block_split_baseline))
block_split_baseline <- c(image_split_baseline,
                          sum(btest_labels != 0)/
                            length(btest_labels))
```
...
model_comp_b <- cbind(cv_lr_b$CV_loss, cv_lda_b$CV_loss, 
                      ...
                      cv_gbm_b$CV_loss, block_split_baseline)
colnames(model_comp_b) <- c("LR", "LDA", "QDA", "Tree", 
                            "10-NN", "RF", "GBM", "Baseline")
model_comp_b %>%
  kable(digits = 3,
