...
Looking at the results from part 4(a), AdaBoost appears to be one of the stronger classifiers for predicting cloudless pixels, but a better classifier would have a negative predictive value closer to the positive predictive value of the model (for the image split the NPV was $19\%$ lower, and for the block split the NPV was $8\%$ lower). A better classifier, either similar to AdaBoost with a different loss function, such as "huberized loss", might be more effective at handling this. Conversely, a classifier like soft-margin SVM might be a better classifier on this dataset, given it might be more robust to predicting future data correctly and predicting unlabeled points in the current dataset, but training SVM was computationally prohibitive for this analysis.
