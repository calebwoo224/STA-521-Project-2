...
b_cv$`NDAI Range` <- cut(b_cv$NDAI, breaks = 5)
b_cv$`CORR Range` <- cut(b_cv$CORR, breaks = 5)
b_cv$`BF Range` <- cut(b_cv$BF, breaks = 5)
...
```{r image-features-errors}
isplit_labeled$`NDAI Range` <- cut(isplit_labeled$NDAI, breaks = 5)
isplit_labeled$`CORR Range` <- cut(isplit_labeled$CORR, breaks = 5)
isplit_labeled$`BF Range` <- cut(isplit_labeled$BF, breaks = 5)
...
i_maps_folds[[2]] <- isplit_labeled %>%
  ...
  geom_point() +
  coord_quickmap(expand = F, clip = "on") +
  labs(title = "Image 2", color = "Fold") +
  scale_color_manual(labels = c("Test Set", "4", "5"),
...
obj1 = arrangeGrob(blank, leg_response, leg_confusion,
             nrow = 1, ncol = 3)
