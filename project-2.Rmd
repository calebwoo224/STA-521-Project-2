...
block_train_labels = tl[labels_block %in% bks[1:132]]
block_valid_labels = tl[labels_block %in% bks[133:162]]
block_test_labels = tl[labels_block %in% bks[163:192]]

block_train = td %>% filter(block %in% bks[1:132])
block_valid = td %>% filter(block %in% bks[133:162])
block_test = td %>% filter(block %in% bks[163:192])
...
```{r different-splitting-CV}
source("CVmaster.R")
source("image_split.R")

CVmaster(data[,-4], data[,4], "lda", K = 8, split = "block", type = "class")
CVmaster(data[,-4], data[,4], "lda", K = 8, split = "image", type = "class")
```
...
CVmaster(X_cv[, -c(4, 6, 8, 9, 11, 12)], y_cv, "glm",
         K = 8, split = "image", type = "response",
         thresh=0.5, family="binomial")
```

```{r ROC-computations, message=FALSE}
im_split <- image_split(data[, -4], data[, 4], 5)
train_data <- cbind(im_split$training_data[, -c(1:3, 12)], im_split$train_labels)
...
logi1 <- glm(Label ~ ., family = "binomial", data = train_data)
preds1 <- predict(logi1, train_data[, -ncol(train_data)], type = "response")
roc1 <- roc(train_data$Label, preds1)
auc1 <- rep(roc1$auc, length(roc1$specificities))

logi2 <- glm(Label ~ NDAI + CORR + BF, family = "binomial", data = train_data)
preds2 <- predict(logi2, train_data[, -c(2, 4:5, 7:9)], type = "response")
roc2 <- roc(train_data$Label, preds2)
auc2 <- rep(roc2$auc, length(roc2$specificities))
```

```{r ROC-comparison}
roc_data <- data.frame(cbind(c(roc1$specificities, roc2$specificities),
                             c(roc1$sensitivities, roc2$sensitivities),
                             c(auc1, auc2)))
model_labels <- c(paste0("LR all variables AUC = ", round(roc1$auc, 3)),
                  paste0("LR 3 AUC = ", round(roc2$auc, 3)))

ggplot(data = roc_data, aes(x = X1, y = X2, color = factor(X3))) +
  geom_line() +
  scale_x_reverse() +
  scale_color_manual(labels = model_labels,
                     values = c("red", "blue")) +
  labs(title = "ROC Model Comparison",
...
CVmaster(btrain, blabel, "lda",
         K = 8, split = "done", type = "class", formula = FALSE)
